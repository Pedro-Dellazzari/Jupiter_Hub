...
export default function Gantt() {
  ...
  return (
    <div className="h-full overflow-y-auto px-10 py-8">
      <h1 className="mb-6 text-[24px] font-bold tracking-[-0.5px] text-(--color-ink)">Gantt</h1>
      ...
      {state.status === "ready" && state.items.length === 0 && (
        <EmptyState
          ...
          description="O Gantt aparece assim que você tiver projetos ou tarefas com prazo definido."
      ...
      {state.status === "ready" && state.items.length > 0 && (
        <div className="flex h-full items-center justify-center">
          <p className="max-w-sm text-center text-[13px] text-(--color-ink-muted)">
            ...
            {state.items.length > 1 ? "s" : ""} — a visualização completa do Gantt ainda não está
            disponível.
          </p>
        </div>
      )}
